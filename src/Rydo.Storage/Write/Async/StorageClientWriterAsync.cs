namespace Rydo.Storage.Write.Async
{
    ...
    internal sealed class StorageClientWriterAsync : IStorageClientWriterAsync
    {
        ...
        public ValueTask UpsertAsync<T>(string key, T payload, FutureWriteResponse futureWriteResponse) =>
            InternalUpsertAsync(key, string.Empty, payload, futureWriteResponse);

        public ValueTask
            UpsertAsync<T>(string key, string sortKey, T payload, FutureWriteResponse futureWriteResponse) =>
            InternalUpsertAsync(key, sortKey, payload, futureWriteResponse);
